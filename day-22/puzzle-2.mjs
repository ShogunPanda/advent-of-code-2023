...
function neighbours(space, brick, whitelist, z) {
  ...
  for(let x = brick[0][0]; x <= brick[1][0]; x++) {
    for(let y = brick[0][1]; y <= brick[1][1]; y++) {
      ...
      if(typeof candidate === 'number' && !whitelist.includes(candidate)) {
...
export async function main() {
  ...
  for(let i = 0; i < bricks.length; i++) {
    ...
    while(queue.length) {
      ...
      const brick = bricks[current]
      const z = Math.max(brick[0][2], brick[1][2]) + 1
  
      // Find all the neighbours above
      removed.add(current)
      const aboves = neighbours(space, brick, [current], z)

      // For all the neighbours, check if there is any supporting block which has not been removed
      for(const above of aboves) {
        const below = neighbours(space, bricks[above], Array.from(removed), z - 1)

        // Nothing is sustaining it, add to the removed queue
        if(below.length === 0) {
          queue.push(above)
        }
      }
    ...
    sum += removed.size - 1 // Do not count the initial block
